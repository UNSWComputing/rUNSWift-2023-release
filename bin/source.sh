...
# get major distro version, e.g. 20 for focal, 22 for jammy, etc
UBUNTU_VER=`lsb_release -r -s | sed  's/^\([0-9]*\).*$/\1/g'`
...
function setupnative() {
    aptinstall libboost-python-dev libboost-program-options-dev libboost-test-dev
    if (( $UBUNTU_VER >= 20 )); then
        aptinstall libqglviewer-dev-qt5 libqwt-qt5-dev
    else
        aptinstall libqglviewer-dev-qt4 libqwt-dev
    fi
    aptinstall libmsgpack-dev libprotobuf-dev libbz2-dev protobuf-compiler cmake g++ libpng-dev libjpeg-dev libboost-system-dev libboost-thread-dev libboost-serialization-dev libboost-regex-dev git
}
...
function tempvirtualenvinstallblack() {
    ...
    python3 -m pip install black[python2]==21.9b0
    python3 -m pip install --upgrade click==8.0.2
    deactivate
}
