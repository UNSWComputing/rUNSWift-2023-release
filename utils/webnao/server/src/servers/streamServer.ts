...
import { connectToHost, setMask, streamBlackboardBytesFromBBD2File, streamBlackboardBytesFromSocket } from '../utils/blackboardStreaming';
...
export class StreamServer {
  ...
  constructor(protected settings: { dumpFilePath: string}, protected robotMonitor: RobotMonitor, protected dumpMonitor: DumpMonitor) {
        ...
        ws.on('message', async (rawCommand: string) => {
          ...
          switch (parsedCommand.action) {
            case 'next': {
              ...
              } else { // reached the end of the stream
  ...
  public async robotStreamEndpoint(robotName: string, type: RobotEndpointType, streamMask?: number): Promise<string> {
    ...
    if (!robotWSStream) {
      ...
      robotWSStream = {
        server: websocketServer,
        socket,
        host, port, type, currentMask: mask, requiedMask: mask,
        clients: 0
      };
