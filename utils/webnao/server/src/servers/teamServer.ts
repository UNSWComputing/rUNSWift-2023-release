...
import { DumpStreamingRequestParams, RobotStreamingRequestParams, StreamingRequestResponse, TeamDataRequest, TeamDataResponse } from '@common/models/teamAPI';
...
export class TeamServer {
  ...
  constructor(protected streamServer: StreamServer, protected robotMonitor: RobotMonitor, protected dumpMonitor: DumpMonitor) {
    this.dumpState = [];
    this.robotState = [];
    this.WSClients = [];
  }
    ...
    return async (data: RawData, isBinary: boolean) => {
      ...
      try {
          ...
          case '/robots':
            {
              const response: TeamDataResponse<StreamingRobotInfo[]> = {
                nonce, requestPath, response: robotState,
              };
            ...
            case '/stream':
              {
                ...
                const response: TeamDataResponse<StreamingRequestResponse> = {
                  nonce, requestPath, response: {endpoint: wsEndpoint},
                };
                connection.send(JSON.stringify({
                  robotState,
                  response,
        ...
        const response: TeamDataResponse = {
          nonce, requestPath, response: robotState, error: (error as Error).message
        };
        connection.send(JSON.stringify({
          robotState,
          response,
