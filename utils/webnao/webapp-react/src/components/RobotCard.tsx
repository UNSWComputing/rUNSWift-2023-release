...
export default function RobotCard({ robot }: RobotCardProps) {
  ...
  const [connectionRequest, setConnectionRequest] = React.useState<RobotConnectionRequest>({robot, type: 'ethernet', maskFlags: defaultMaskFlags});
  ...
  type maskCheckboxHandler = (flagName: keyof IStreamMaskFlags) => (e: ChangeEvent<HTMLInputElement>, checked: boolean) => void;
  const maskChanged: maskCheckboxHandler = (flagName: keyof IStreamMaskFlags) => (e: ChangeEvent<HTMLInputElement>, checked: boolean) => {
    ...
    <React.Fragment>
        ...
        <DialogContent>
          ...
          <FormGroup>
            <FormControlLabel control={<Checkbox
              checked={connectionRequest?.maskFlags.blackboard}
              onChange={maskChanged('blackboard')}
            />} label="Blackboard" />
            <FormControlLabel control={<Checkbox
              checked={connectionRequest?.maskFlags.saliency}
              onChange={maskChanged('saliency')}
            />} label="Saliency" />
            <FormControlLabel control={<Checkbox
              checked={connectionRequest?.maskFlags.rawImage}
              onChange={maskChanged('rawImage')}
            />} label="Raw image" />
          </FormGroup>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', padding: '5px'}}>
        <Box sx={{display: 'flex', flexDirection: 'row'}}>
          <div>{robot.name} T{robot.teamInfo?.teamNumber} P{robot.teamInfo?.playerNumber}</div>
