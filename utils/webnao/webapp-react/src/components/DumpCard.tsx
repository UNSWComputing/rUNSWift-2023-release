    ...
    <React.Fragment>
      <Box sx={{ display: 'flex', flexDirection: 'column', padding: '5px'}}>
        <Box sx={{display: 'flex', flexDirection: 'column'}}>
          <div>{dumpName}</div>
          { dump.teamInfo ?
            <div>T{dump.teamInfo?.teamNumber} P{dump.teamInfo?.playerNumber}</div>
            : null 
          }
          {
            dump.timestamps ?
            <div>{new Date(dump.timestamps[0]/1000).toLocaleDateString()} {new Date(dump.timestamps[0]/1000).toLocaleTimeString()}</div>
            : null
          }
          {
            dump.lengthSeconds ?
            <div>{dump.lengthSeconds.toFixed(0)}s</div>
            : null
          }
        </Box>
