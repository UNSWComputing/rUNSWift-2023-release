...
//                    readFrom(stateEstimation, sharedStateEstimationBundle),
   ...
   u_int16_t packet_hard_limit = 50; // 2023 limit is 1200
   if( ((u_int16_t)readFrom(gameController, our_team).messageBudget) > packet_hard_limit)
   {
      NaoTransmitter::tick(boost::asio::buffer(&m, sizeof(SPLStandardMessage)));
   }
   else
   {
      llog(INFO) << "WE'RE RUNNING OUT OF PACKETS!!!" << endl;
   }
