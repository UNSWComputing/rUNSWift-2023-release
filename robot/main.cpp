...
//#endif
  ...
  if (!vm.count("simulation") && vm["debug.vision"].as<bool>()) {
   ...
   // changing the below TeamTransmitter value (indicating time interval in micro seconds to send a new packet)
            // would also require change to SECS_TILL_INCAPACITATED value in incapacitated.hpp
   ThreadManager teamTransmitter("TeamTransmitter", 8000000); // 0.125fps limit - helps 7 Naos stay under the 1200 packets per ~21.5 min game limit (21.5*60)/(1200/7)
