...
class Config(object):
    ...
    def __init__(self):
        ...
        self.spectrum_whistle_begin = self.fWhistleBegin * self.window_size // self.fSampleRate
        self.spectrum_whistle_end = self.fWhistleEnd * self.window_size // self.fSampleRate
...
def check_on_nao():
    ...
    """
    try:
        cmd = subprocess.Popen(["lsb_release", "-a"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except OSError:
        return False
    data = cmd.communicate()[0]
    lines = data.split("\n")
    has_nao = [line for line in lines if "Nao" in line]
    return has_nao
...
def check_runswift_running():
    ...
    lines = data.split("\n")
    has_runswift = [line for line in lines if "runswift" in line]
...
def timethis(func):
    ...
    def wrap(*args, **kwargs):
        ...
        try:
            ...
            if VERBOSITY >= 2:
                msg = "{}: Took {:.3f} seconds".format(func.__name__, time.time() - start)
                print(msg)
...
def debug_vis(spectrum, mean, st_dev):
    ...
    as_buckets = [sum(spectrum[i * buckets : (i + 1) * buckets]) // buckets for i in range(len(spectrum) // buckets)]
    stdout.write("\n__")
...
class WhistleState(object):
    ...
    def interrogate(self, complex_spectrum):
        ...
        whistle_threshold = spectrum_mean + _config.spectrum_threshold * spectrum_st_dev
        if VERBOSITY >= 2:
            stdout.write(".")
            stdout.flush()
        ...
        background_growth_threshold = spectrum_mean + _config.background_threshold * spectrum_st_dev
        ...
        if state.whistleDone:
            ...
            if found:
                if VERBOSITY >= 2:
                    ...
                    print("C {} - MC {}".format(state.whistleCounter, state.whistleMissCounter))
...
class SoundReceiverMixin(object):
    ...
    def find_whistle(self, sound_data, num_channels, time_stamp=None):
        ...
        while processed < len(signal):
            ...
            if VERBOSITY >= 3:
                print("{:.3f} seconds".format(self.sound_time / _config.spectra_per_second))
            self.sound_time += 1.0
    ...
    def to_sound_data(audio_buffer, num_channels, samples_per_channel):
        sound_data_interlaced = numpy.fromstring(str(audio_buffer), dtype=numpy.int16)
        sound_data = numpy.reshape(sound_data_interlaced, (num_channels, samples_per_channel), "F")
        return sound_data
...
if ON_NAO_ROBOT:
    ...
    class SoundReceiverModule(SoundReceiverMixin):
        ...
        def process_buffer(self, num_channels, samples_per_channel, time_stamp, audio_buffer):
            """
            ...
            keep = _config.sample_rate * keep_seconds * num_channels * encoded_amplitude
            if bytes_so_far > keep:
                self.last_2_secs_buffers.pop(0)
            self.last_2_secs_buffers.append(audio_buffer)

            sound_data = self.to_sound_data(audio_buffer, num_channels, samples_per_channel)
    ...
    def wav_test(self, in_file_name):
        ...
        while read < total_frames:
            ...
            if len(audio_buffer) < _config.window_size:
                if VERBOSITY >= 2:
                    print("Warning: Skipping unexpected buffer size {}".format(len(audio_buffer)))
                continue
            sound_data = self.to_sound_data(audio_buffer, num_channels, samples_per_channel)
...
if __name__ == "__main__":
    ...
    def write_to_file_with_stack_trace(message, error):
        if not os.path.exists(NAO_WHISTLE_LOCATION):
            os.makedirs(NAO_WHISTLE_LOCATION)
        with open(NAO_WHISTLE_LOCATION + "/whistle_log.txt", "a") as outfile:
            outfile.write("-" * 20 + datetime.now().isoformat() + "-" * 20 + "\n")
            exc_name = getattr(getattr(error, "__class__", error), "__name__", "")
            outfile.write(exc_name + ": " + unicode(error) + "\n")
    ...
    while ON_NAO_ROBOT and RETRY > 0:
        ...
        try:
            ...
            write_to_file_with_stack_trace("Crash caught, attempting to kill " "pulseaudio to see if it fixes it.", e)
